...
- (id)initWithNibName:(NSString *)nibNameOrNil bundle:(NSBundle *)nibBundleOrNil
{
    ...
    if (self) {
        // 子页面初始化
        _leftViewShowWidth = 267;  // 左抽屉滑动距离
        _rightViewShowWidth = 267; // 右抽屉滑动距离
        _animationDuration = 0.35; // 动画持续时间按
        ...
        [_coverButton addTarget:self action:@selector(hideSidetViewController:) forControlEvents:UIControlEventTouchUpInside];
...
- (void)viewDidLoad {
    ...
    _baseView = self.view;
//    [_baseView setBackgroundColor:[UIColor colorWithRed:.5 green:.6 blue:.8 alpha:1]];
...
- (void)hideSidetViewController:(BOOL)animated{
...
//    [self hideSideViewController:true];
    [self hideSidetViewController:true];
}
